...
public:
    PuffCurrent(int numCha, int numCls, float rateOpnSingle, float rateRef, float rateCls, double ip3, double ca0,
                double caT) :
...
int main(int argc, char *argv[]) {
    ...
    out_file_spikes = path + "transient_spike_times_langevin" + parameters;
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        while (count_spikes < 25) {
            ...
            if (ci >= k_cT) {
                ...
                ISIs[count_spikes] = t_since_spike;
                count_spikes += 1;
                t_since_spike = 0;
            }
            t += dt;
        ...
        for (auto &interval: ISIs) {
            file_spikes << fixed << setprecision(2) << interval << " ";
        }
        file_spikes << endl;
    }
    return 0;
