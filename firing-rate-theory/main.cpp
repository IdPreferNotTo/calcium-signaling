...
public:
    ...
    void set_derivations_intesities() {
        ...
        for (int i = 0; i < imax_ + 1; i++) {
            D_left = intensity((i - 1) * dCa_);
            D_right = intensity((i + 1) * dCa_);
            derivativesIntesities_ca = (D_right - D_left) / (2 * dCa_);
            derivatives_intesities_.push_back(derivativesIntesities_ca);
        }
    }
    ...
    double get_mean(double ca) const {
        ...
        mean = means_[idx] + (means_[idx + 1] - means_[idx]) * (ca - ca_at_idx) / dCa_;
        return mean;
    }
    ...
    double get_intensity(double ca) const {
        ...
        // linear interpolation
        intensity = intensities_[idx] + (intensities_[idx + 1] - intensities_[idx]) * (ca - ca_idx) / dCa_;
        return intensity;
    }
    ...
    double get_derivative_intensity(double ca) const {
        ...
        // linear interpolation
        derivative = derivatives_intesities_[idx] +
                     (derivatives_intesities_[idx + 1] - derivatives_intesities_[idx]) * (ca - ca_idx) / dCa_;
        return derivative;
    }
    ...
    double firing_rate(float tau, float j, float cer, int N, float a) const {
...
int main(int argc, char *argv[]) {
    ...
    double r0 = jpuff.firing_rate(tau, j, cer, N, a);

    // Insert if run as main

    /*
    std::string path;
    path = std::string(homedir) + "/Data/calcium_spikes_theory/firing_rate/";
    char parameters[200];
    std::sprintf(parameters, "r0_self_ip%.2f_tau%.2e_j%.2e_tauer_%.2e_epser%.2e_K%d.dat", ip3, tau, j, tau_er, eps_er, N);
