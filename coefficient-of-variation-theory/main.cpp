...
public:
    ...
    void set_intensities() {
        ...
        double intensity_ca;
        for (int i = 0; i < imax_+1; i++) {
            intensity_ca = intensity(i * d_ca_);
    ...
    void set_derivations_intesities() {
        ...
        for (int i = 0; i < imax_ +1; i++) {
            d_left = intensity((i - 1) * d_ca_);
            d_right = intensity((i + 1) * d_ca_);
            derivatives_intesities_ca = (d_right - d_left) / (2 * d_ca_);
    ...
    double get_mean(double ca) const{
        ...
        idx = int(imax_*(ca/ca_t_));
        ca_at_idx = idx * d_ca_;

        // linear interpolation
        mean = means_[idx] + (means_[idx+1] - means_[idx]) * (ca - ca_at_idx) / d_ca_;
        return mean;
    }
    ...
    double get_intensity(double ca) const{
        ...
        // linear interpolation
        intensity = intensities_[idx] + (intensities_[idx+1] - intensities_[idx]) * (ca - ca_idx) / d_ca_;
        return intensity;
    }
    ...
    double get_derivative_intensity(double ca) const{
        ...
        // linear interpolation
        derivative = derivatives_intesities_[idx] + (derivatives_intesities_[idx + 1] - derivatives_intesities_[idx]) * (ca - ca_idx) / d_ca_;
        return derivative;
    }

    double f_func(double ca, float tau, float j, int N, float a) const{
        if(ca == 0){
            return -(ca - ca_r_) / tau;
        }
        else{
            ...
            return -(ca - ca_r_) / tau + j * N * f + a * j * j * N * der_d;
        }
    }

    double d_func(double ca, float tau, float j, int N) const{
        if(ca == 0){
            return 0;
        }
        else{
    ...
    double g_func(double ca, float tau, float j, int N, float a) const{
        double f, d;
        f = f_func(ca, tau, j, N, a);
        d = d_func(ca, tau, j, N);
        return f/d;
    }

    double h_func(double ca, float tau, float j, int N, float a) const{
    ...
    double firing_rate(float tau, float j, int N, float a) const{
        std::vector<double> cas;
        cas = range(0.5, 0.2, 28001);
    ...
    double cv_arecchi(float tau, float j, int N, float a){
        std::vector<double> cas;
        cas = range(0.5, 0.2, 28001);
        double dca = cas.at(0) - cas.at(1);
        double integral = 0;
        ...
        double r0 = firing_rate(tau, j, N, a);

        for (auto& ca: cas){
            f = f_func(ca, tau, j, N, a);
            d = d_func(ca, tau, j, N);

            integral += dca*2*d/(f*f*f);
        };
        return r0*sqrt(integral);
    };

    double cv2(float tau, float j, int N, float a) const {
        std::vector<double> cas;
        cas = range(0.15, 0.5, 1001);

        double dca = cas.at(1) - cas.at(0);

        std::vector<double> p0s_no_norm;
        std::vector<double> p0s;
        std::vector<double> I2s;
        ...
        for(auto &ca: cas){
            cout << ca << endl;
            std::vector<double> cas1 = range(ca, 0.5, 1001);
            double dca1 = cas1.at(1) - cas1.at(0);
            I1 = 0;
            for(auto &ca1: cas1){
                if(ca1 > ca_r_){
                    h = h_func(ca1, tau, j, N, a);
                    I1 += exp(-h)*dca1;
                }
                else{
                    I1 += 0;
            ...
            std::vector<double> cas2 = range(0.1, ca, 1001);
            double dca2 = cas2.at(1) - cas2.at(0);
            I2 = 0;
            for(auto &ca2: cas2) {
                h = h_func(ca2, tau, j, N, a);
                d = d_func(ca2, tau, j, N);
                I2 += (exp(h)/d)*dca2;
            };

            h = h_func(ca, tau, j, N, a);
            I3 += 2*exp(-h)*I2*I2*I1*dca;
        }
        double r0 = firing_rate(tau, j, N, a);
        return r0*r0*I3;
...
int main(int argc, char *argv[]) {
    ...
    std::string path;
    path = std::string(homedir) + "/Data/calcium/theory/";
    char parameters[200];
    std::sprintf(parameters, "cer_cv_fpe_tau%.2e_j%.2e.dat", tau, j);
    ...
    std::vector<double> cers = range(0.8, 1, 50);
    for (auto& cer: cers){
        cout << cer << endl;
        j_puff jpuff(N, M, r_opn_single, r_ref, r_cls, ip3, caR, caT);
        double cv2 = jpuff.cv2(tau, j*cer, K, a);
        double cv = sqrt(cv2);
        file << cer << " " << cv << endl;
    }
    ...
    //double cv_arecchi = jpuff.cv_arecchi(tau, j, K, a);
    //cout << cv << " " << cv_arecchi << endl;
