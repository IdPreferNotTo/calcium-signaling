...
int main(int argc, char *argv[]) {
    ...
    double interspike_intervals[n_max];
    ...
    while(i < n_max){
        rng = dist(generator);
        j_leak = -(ci - ciR) / tau_i;
        m_j_puff = j * K * j_puff.getMean(ci);
        sqrt2d_j_puff = j * sqrt(2 * K * j_puff.getIntensity(ci));
        s_drift_j_puff = (1. / 2.) * pow(j, 2) * K * j_puff.getDerivativeIntensity(ci);
        n_j_puff = sqrt2d_j_puff * rng;

        ci += (j_leak + m_j_puff + s_drift_j_puff) * dt + n_j_puff * sqrt(dt);
        if (ci >= ciT) {
            ci = ciR;
            interspike_intervals[i] = t_since_spike;
            t_since_spike = 0;
            i++;
        }
        t += dt;
    ...
    for (double interspike_interval : interspike_intervals){
            cout << interspike_interval << " ";
    }
    return 0;
}
